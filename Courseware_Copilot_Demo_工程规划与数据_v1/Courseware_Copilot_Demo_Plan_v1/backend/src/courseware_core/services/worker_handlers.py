"""worker handler 装配（core 层单一事实源）：Web 与 CLI 两个入口共用。

原位于 courseware_api/wiring；T13 CLI（courseware_core.cli）需要同一套
受理+执行装配，而 core 不得反向依赖 api 层，故装配下沉到 core。
handler 在 worker 线程内使用独立 SQLite 连接。
"""
