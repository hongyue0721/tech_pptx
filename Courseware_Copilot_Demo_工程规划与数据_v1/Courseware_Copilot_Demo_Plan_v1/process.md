...
## 当前任务
**T06 已提交（commit d0a0e5e）。T07（APP 模型 Adapter 与预算）实现完成并通过独立 Review 修复闭环（B1/N1/N3/N4/N6/N7 修复、N2/N5 注释固化进 T08，报告 docs/reviews/t07-review.md），pytest 353 全绿，待提交。T07 剩余验收项：真实协议 smoke——属付费动作，须负责人预算授权后执行（小岳已准备好探测清单，未批准前零真实调用）。下一步：T08（上下文组装与提示词治理）**。backlog：T04-Review N4/N6/N9/N10；T05-Review N8(PageLabels 差距)/N10(矢量页)/N12(duplicate 等待把手)/N16(解析超时)；T07-Review N2(JobContext project/revision/trace 字段)/N5(CallBudget 并发锁，T08 并行 stage 前)；更早：N7 schema enum 补 type:string、AnyUI 图标本地静态打包（正式接入时）。仓库分支master，最新commit d0a0e5e（T07 改动未提交）。
